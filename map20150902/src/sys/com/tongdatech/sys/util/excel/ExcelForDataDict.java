...
public class ExcelForDataDict extends ExcelBean {

	public ExcelForDataDict(int excelEdition) {
		super(excelEdition);
	...
	protected void bulidTitle() {
		...
		r0.setHeightInPoints(15);
		Cell c0 = r0.createCell(0);
		c0.setCellValue(createHelper.createRichTextString(tablename));
		sheet.addMergedRegion(new CellRangeAddress(rownum,rownum,0,colnum-1));
		CellStyle cs0 = wb.createCellStyle();
		cs0.setAlignment(CellStyle.ALIGN_CENTER);
		cs0.setVerticalAlignment(CellStyle.VERTICAL_CENTER);
		Font ff=wb.createFont();
		ff.setFontName("宋体");
		ff.setBoldweight(Font.BOLDWEIGHT_BOLD);
	...
	protected void bulidData() {
		...
		odd.setWrapText(true);//设置自动换行
		even.setWrapText(true);
