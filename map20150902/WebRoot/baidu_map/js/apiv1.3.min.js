(function () {//乱码测试
	...
	a2.string.format = function (cF, T) {
		cF = String(cF);
		var cE = Array.prototype.slice.call(arguments, 1),
	...
	a2.dom.getStyle = function (cF, cE) {
		...
		var cH = cF.style[cE];
		if (!cH) {
			var T = cI._styleFixer[cE],
			cG = cF.currentStyle || (a2.browser.ie ? cF.style : getComputedStyle(cF, null));
	...
	a2.dom.getPosition = function (T) {
		T = a2.dom.g(T);
		var cM = a2.dom.getDocument(T),
		cG = a2.browser,
		cJ = a2.dom.getStyle,
		cF = cG.isGecko > 0 && cM.getBoxObjectFor && cJ(T, "position") == "absolute" && (T.style.top === "" || T.style.left === ""),
		cK = {
			left : 0,
			top : 0
		},
		cI = (cG.ie && !cG.isStrict) ? cM.body : cM.documentElement,
		cN,
		cE;
		if (T == cI) {
			return cK
		...
		if (T.getBoundingClientRect) {
			...
			var cL = cM.body,
			cO = parseInt(cJ(cL, "borderLeftWidth")),
			cH = parseInt(cJ(cL, "borderTopWidth"));
	...
	a2.extend(bt.prototype, {
		...
		getBounds : function () {
			...
			var cE = arguments[0] || {},
			cG = cE.margins || [0, 0, 0, 0],
			T = cE.zoom || null,
			cH = this.pixelToPoint({
					x : cG[3],
					y : this.height - cG[2]
				}, T),
		...
		getViewport : function (cM, cE) {
			...
			if (cE.margins) {
				...
				cK = (cL[1] - cL[3]) / 2,
				cN = (cL[0] - cL[2]) / 2,
				cG = this.getMapType().getZoomUnits(cP);
				T.lng = T.lng + cG * cK;
				T.lat = T.lat + cG * cN
			}
	...
	function bD(T) {
		...
		this._opts = a2.object.extend(a2.object.extend(this._opts, {
					color : "black",
					unit : "metric"
				}), T);
		this.defaultAnchor = BMAP_ANCHOR_BOTTOM_LEFT;
	...
	a2.extend(bZ, {
		...
		getDgPix_mm : function (cH, cM, cI) {
			...
			var cQ = cJ.x,
			T = cJ.y;
			var cO = this.correct_pts[cH][cM].utm_x,
			cG = this.correct_pts[cH][cM].utm_y;
			var cK = this.correct_pts[cH][cI].utm_x,
			cF = this.correct_pts[cH][cI].utm_y;
	...
	bt.prototype.openInfoWindow = function (cG, T) {
		...
		var cE = this.temp;
		if (!cE.marker) {
			var cF = new K(b5.imgPath + "blank.gif", {
					width : 1,
					height : 1
				});
			cE.marker = new aa(T, {
	...
	a2.extend(a.prototype, {
		...
		_getPerimeterPoints : function (T, cL) {
			...
			for (var cJ = 0; cJ < 360; cJ += 9) {
				var cH = (Math.PI / 180) * cJ,
				cO = Math.asin(Math.sin(cK) * Math.cos(cN) + Math.cos(cK) * Math.sin(cN) * Math.cos(cH)),
				cM = Math.atan2(Math.sin(cH) * Math.sin(cN) * Math.cos(cK), Math.cos(cN) - Math.sin(cK) * Math.sin(cO)),
				cP = ((cQ - cM + Math.PI) % (2 * Math.PI)) - Math.PI,
				cR = new b6(cP * (180 / Math.PI), cO * (180 / Math.PI));
				cS.push(cR)
			}
			var cF = cS[0];
	...
	ay.prototype.getTilesUrl = function (cJ, cE) {
		...
		cL = cJ.x,
		cG = cJ.y,
		cI = Math.floor(cL / 200),
		cH = Math.floor(cG / 200),
		T = this._tileUrl + cK + "level=" + cE + "&x=" + cL + "&y=" + cG;
	...
	aO.getTilesUrl = function (cF, cI) {
		...
		cE = "maptile_test/" + cI + "/" + cJ + "/" + cG + ".jpg";
		return cE.replace(/-(\d+)/gi, "M$1");
	};
	window.BMAP_NORMAL_MAP = new cn("\u5730\u56fe", aO, {
			tips : "\u663e\u793a\u666e\u901a\u5730\u56fe"
		});
	var bm = new n();
	...
	var a5 = {
		...
		request : function (cE, cK, T, cJ) {
			...
			var cI = this,
			cG = cI.REQ_BASE_URL + cJ + "?" + cH + "&ie=utf-8&oue=1&res=api&callback=BMap._rd._cbk" + cF;
			cq.request(cG)
		}
	};
	BMap._rd = {};
