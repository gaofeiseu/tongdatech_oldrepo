			...
			function doQuery(){
			   if($('#sn').combobox('getValue')=="")
        	    {
        	    	  $.messager.show({title: "信息",msg: "请先选择小类再进行查询"});
        	    	  return; 
        	    } 
				var cols = [];
			    ...
			    var url="/map_data_doquery.action?SN="+$('#sn').combobox('getValue');
			...
			function getselect(newValue,oldValue)
			{
			 	  $('#sn').combobox('clear');
			      $('#sn').combobox('reload', "/map_data_getSN.action?marker_class="+newValue);   
			}
			var colMap;
			function getColumn(newValue,oldValue)
			{
				var json = {};
				json["SN"]=newValue; 
				json["marker_class"]=$('#marker_class').combobox('getValue'); 
				var url="/map_data_queryColumn.action";
				$.ajax({
					type : "POST",
					cache : false,
					async:true,
			...
			function doSave(){ 
				...
				var SN =sn;
				$("#user_fm").attr("action","");
				var urls="/map_data_doSave.action?SN="+sn+"&query_operator="+filecolumn;
			 		$("#user_fm").form("submit",{
			    	url: urls,
					onSubmit: function(){
...
function expportExcel()
{
	...
	//expexcel('#dataTab','/map_data_excel.action');
	...
	excelParam['url']='/map_data_excel.action';
